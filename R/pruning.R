...
.prune_network_step <- function(network_bef,
  ...
  res_edge_eval_single_step <- parallel::mclapply(var_to_prune_vec, function(j){
    ...
    R2_j_multi_mat <- R2_multi_arr[,j,]  # slice j, a matrix of dim (p, K)
    R2_j_avg_vec <- R2_avg_mat[,j]  # slice j, a vector of length p
    for (var_idx in regulator_var_idx_vec){
      ...
      R2_j_multi_mat[var_idx, ] <- R2_temp_vec
      R2_j_avg_vec[var_idx] <- mean(R2_temp_vec)
    }

    # prune the least important edge for this variable
    R2_j_min <- min(R2_j_avg_vec, na.rm = T)  # the least important selected regulator

    if (R2_j_min < prune_thres) {
      # at least one edge can be pruned. Prune the least important regulator.
      var_idx_pruned <- which(R2_j_avg_vec == R2_j_min)[1]
...
prune_network <- function(network_original,
                          prune_thres = 0.05,
                          depth = NULL,
                          eval_edge_R2_pruned = FALSE,  # whether to evaluate each edge in the final pruned network
                          Y_list,
                          yy_smth_list,
                          obs_time_list,
                          tt,
  ...
  if (!eval_edge_R2_pruned){
    ...
    res_edge_eval <- parallel::mclapply(1:p, function(j){
      ...
      R2_j_multi_mat_pruned <- R2_multi_arr_pruned[,j,]  # slice j, a matrix of dim (p, K)
      R2_j_avg_vec_pruned <- R2_avg_mat_pruned[,j]  # slice j, a vector of length p
      for (var_idx in regulator_var_idx_vec){
        ...
        # update the R2 results
        R2_j_multi_mat_pruned[var_idx, ] <- R2_temp_vec
        R2_j_avg_vec_pruned[var_idx] <- mean(R2_temp_vec)
